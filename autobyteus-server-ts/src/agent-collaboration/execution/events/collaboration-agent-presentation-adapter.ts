...
export type ResolvePresentationIdentityByAgentRunId = (agentRunId: string) => CollaborationMemberExecutionIdentity | null;
...
const correlated = <T extends AgentPresentationEvent>(event: T): Readonly<{ kind: "publish"; event: T }> =>
...
type PresentationFileChangeDetails = Extract<AgentPresentationEvent, { eventType: "FILE_CHANGE" }>["details"];

const fileChange = (event: AgentRunEvent): PresentationFileChangeDetails => {
...
const tokenRunSummary = (
...
): Omit<TokenUsageRunSummaryPayload, "root_team_run_id"> | null => {
  ...
  if (value === null) return null;
  if (!isRecord(value)) throw new Error("run_summary_after_event is invalid");
  const { root_team_run_id: rootTeamRunId, ...rootNeutral } = value;
...
const token = (payload: Record<string, unknown>, expectedIdentity: CollaborationMemberExecutionIdentity): AgentPresentationTokenUsageDetails => {
  ...
  const claimedRoot = raw(payload, "root_team_run_id", "rootTeamRunId");
...
export class AgentRunPresentationAdapter {
  constructor(private readonly resolveIdentityByAgentRunId: ResolvePresentationIdentityByAgentRunId) {}

  adapt(event: AgentRunEvent): AgentRunPresentationAdaptationResult {
  ...
  private adaptKnown(event: AgentRunEvent): AgentRunPresentationAdaptationResult {
    ...
    switch (event.eventType) {
      ...
      case AgentRunEventType.AGENT_STATUS:
        return correlated({ eventType: "AGENT_STATUS", details: {
      ...
      case AgentRunEventType.TOKEN_USAGE_UPDATED: {
        ...
        if (!identity) throw new Error("run_id does not resolve in the active collaboration root");
      ...
      case AgentRunEventType.SYSTEM_TASK_NOTIFICATION: {
        ...
        const sender = senderRunId
          ? (() => { const identity = this.resolveIdentityByAgentRunId(senderRunId); if (!identity) throw new Error("sender_run_id does not resolve in the active collaboration root"); return Object.freeze({ kind: "execution" as const, identity }); })()
