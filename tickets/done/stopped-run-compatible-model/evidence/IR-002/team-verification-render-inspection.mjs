...
const outputDir = path.resolve(webDir, getArg('output-dir', '../tickets/in-progress/stopped-run-compatible-model/evidence/IR-002/rendered'))
...
// Local transport fixture for the existing verification lane, not API/E2E acceptance.
const state = {
  failVerificationOnce: true,
...
const operationResponse = async (operationName, variables) => {
  ...
  if (operationName === 'GetTeamRunResumeConfig') {
    ...
    if (state.teamResumeReads > 1 && state.failVerificationOnce) {
      state.failVerificationOnce = false
      return { errors: [{ message: 'Canonical Team verification temporarily unavailable.' }] }
    }
  ...
  if (operationName === 'UpdateStoppedTeamRunModelConfigs') {
    ...
    return { data: { updateStoppedTeamRunModelConfigs: {
      ...
      message: 'Update outcome is being verified. Refresh the Team configuration before saving again.',
      isActive: false,
      editability: { editable: true, reason: null },
      canonicalExecutionTree: priorTree,
      fieldErrors: [],
    } } }
  }
...
try {
  ...
  await runScenario('IR-002-RENDER', 'CRF-001 Team indeterminate outcome -> automatic verification failure -> Retry; deterministic transport only', async () => {
    ...
    const modelButton = page.locator('#team-scope-root-runtime-kind').locator('xpath=../following-sibling::div[1]//button').first()
    ...
    await waitFor('failed canonical verification exposes Retry', async () => state.teamResumeReads === 2 && await retry.isEnabled())
    assert(await save.isDisabled(), 'Save remains disabled while verification is required')
    assert(await modelButton.isDisabled(), 'Model edits stay locked until canonical verification')
    assert(state.teamMutations.length === 1, 'Exactly one Save mutation sent')
    await page.screenshot({path:path.join(outputDir,'team-verification-retry.png'),fullPage:true})
    await page.setViewportSize({width:520,height:900})
    await page.screenshot({path:path.join(outputDir,'team-verification-narrow.png'),fullPage:true})
    assert(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth), 'No horizontal overflow at 520px')
    await retry.click()
    await waitFor('retry installs canonical new pair', async () => state.teamResumeReads === 3 && !(await retry.count()) && await modelButton.isEnabled())
    assert(await modelButton.innerText().then(text => text.includes('larger-model')), 'Canonical new model is displayed')
    assert(await save.isDisabled(), 'Canonical verification leaves a clean, non-resubmittable draft')
    assert(!(await page.getByText('Canonical Team verification temporarily unavailable.', {exact:true}).count()), 'Successful verification clears stale error feedback')
    assert(state.teamMutations.length === 1, 'Retry is a read, not a second Save')
    await page.screenshot({path:path.join(outputDir,'team-verified-narrow.png'),fullPage:true})
    return { mutation: state.teamMutations[0], canonicalModel: state.teamTree.root_team.default_launch_configuration.llm_model_identifier,
      canonicalConfig: state.teamTree.root_team.default_launch_configuration.llm_config,
      mutationCount: state.teamMutations.length, canonicalReadCount: state.teamResumeReads,
      expectedInjectedFailure: 'One automatic canonical refresh returns a GraphQL error; Retry succeeds.' }
  })
