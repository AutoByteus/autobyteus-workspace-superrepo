<template>
  <main class="flex h-screen flex-col bg-slate-50 text-slate-900">
    <header class="flex shrink-0 flex-wrap items-center gap-2 border-b p-2 text-xs">
      <strong>IR-045 · isolated Org composer</strong>
...
onMounted(()=>{
  ;(window as any).__ir045=()=>Object.fromEntries((orgs.contextFor('org-run')?.listAgentContextEntries()??[]).map(({agentRunId,context})=>[agentRunId,{draft:context.requirement,attachments:context.contextFilePaths,pending:context.submissionPending,messages:context.conversation.messages}]))
  ;(window as any).__ir045Context=()=>active.activeAgentContext
