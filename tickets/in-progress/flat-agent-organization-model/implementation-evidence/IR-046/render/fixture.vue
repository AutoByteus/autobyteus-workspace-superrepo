<template>
  <main class="flex h-screen flex-col bg-slate-50 text-slate-900">
    <header class="flex shrink-0 flex-wrap items-center gap-2 border-b p-2 text-xs">
      <strong>IR-046 · isolated Org history / continuation / stop</strong>
...
onMounted(()=>{
  ;(window as any).__ir046=()=>Object.fromEntries((orgs.contextFor('org-run')?.listAgentContextEntries()??[]).map(({agentRunId,context})=>[agentRunId,{draft:context.requirement,attachments:context.contextFilePaths,pending:context.submissionPending,messages:context.conversation.messages}]))
  ;(window as any).__ir046Context=()=>active.activeAgentContext
