...
const page = browser.contexts().flatMap(c=>c.pages()).find(p=>p.url().includes('127.0.0.1:3589'));
...
await page.evaluate(() => {
  window.__APIREV10_RECOVERY_PROBE__ = {closes:[],errors:[],rejections:[],installedAt:new Date().toISOString()};
  const original = WebSocket.prototype.close;
  WebSocket.prototype.close = function(code, reason) {
    window.__APIREV10_RECOVERY_PROBE__.closes.push({at:new Date().toISOString(),url:this.url,readyState:this.readyState,code:code ?? null,reason:reason ?? null});
    return original.call(this, code, reason);
  };
  window.addEventListener('error', e=>window.__APIREV10_RECOVERY_PROBE__.errors.push({at:new Date().toISOString(),message:e.message,error:String(e.error||'')}));
  window.addEventListener('unhandledrejection', e=>window.__APIREV10_RECOVERY_PROBE__.rejections.push({at:new Date().toISOString(),reason:String(e.reason),stack:e.reason?.stack||null}));
});
...
for (let i=0;i<112;i++) {
  ...
  if (i%4===0) {
    const state=await page.evaluate(() => ({
      ...
      probe:window.__APIREV10_RECOVERY_PROBE__
    }));
    emit('sample',{i,state});
  }
}
...
emit('final',{final});
console.log(JSON.stringify({outPath:outPath.pathname,url:page.url(),closes:final.probe.closes,errors:final.probe.errors,rejections:final.probe.rejections,notice:final.text.includes('Live updates could not recover automatically'),connecting:final.text.includes('Connecting'),priorMarker:/APIREV(8|10)-ORG-(DIRECT|TEAM)/.test(final.text),stopControl:final.buttons.some(x=>x.aria==='Stop Agent Org'),statuses:final.statuses},null,2));
await page.screenshot({path:new URL('screenshots/LIVE-004-stopped-root-recovery.png',import.meta.url).pathname,fullPage:true}); await cdp.detach(); await browser.close();
