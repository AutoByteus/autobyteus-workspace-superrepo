...
The closed audit inventory is declared in `localization/audit/migrationScopes.ts`. The Agent Org scope (`M-014`) covers its management route, launch configuration, history, workspace state, and return navigation. It uses strict Vue inspection so static text, placeholders, mixed interpolation text, presentation attributes, and script-produced labels or errors cannot bypass the catalog boundary.
